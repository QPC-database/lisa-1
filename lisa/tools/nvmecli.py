...
class Nvmecli(Tool):
    ...
    __nm_management_attachement_support = "NS Management and Attachment Supported"
    # [1:1] : 0x1   Format NVM Supported
    __format_nm_support = "Format NVM Supported"
    ...
    def support_nm_manage_attach(self, device_name: str) -> bool:
        cmd_result = self.run(f"id-ctrl -H {device_name}", shell=True, sudo=True)
        cmd_result.assert_exit_code()
        return self.__nm_management_attachement_support in cmd_result.stdout

    def support_nm_format(self, device_name: str) -> bool:
        cmd_result = self.run(f"id-ctrl -H {device_name}", shell=True, sudo=True)
        cmd_result.assert_exit_code()
        return self.__format_nm_support in cmd_result.stdout

...
class Transformer(subclasses.BaseClassWithRunbookMixin):
    def __init__(self, runbook: schema.Transformer, *args: Any, **kwargs: Any) -> None:
...
def _run_transformers(
    ...
    for runbook in transformers_runbook:
        ...
        transformer = factory.create_by_runbook(runbook=runbook)
...
def run(runbook_builder: RunbookBuilder) -> None:
    ...
    # transformers support variables, and transfomers will update variables.
    variables = runbook_builder.variables
    ...
    mock_variables = _run_transformers(transfromers_data, variables, is_dry_run=True)
    ...
    output_variables = _run_transformers(transfromers_data, variables)
    merge_variables(variables, output_variables)

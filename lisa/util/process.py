...
class ExecutableResult:
    ...
    def assert_exit_code(self, expected_exit_code: int = 0) -> AssertionBuilder:
        return assert_that(
            self.exit_code, f"get unexpected exit code on cmd {self.cmd}"
        ).is_equal_to(expected_exit_code)

    def assert_stderr(self, expected_stderr: str = "") -> AssertionBuilder:
        return assert_that(
            self.stderr, f"get unexpected stderr on cmd {self.cmd}"
        ).is_equal_to(expected_stderr)
